...
import useProductEffect from '../hooks/useProductEffect';
import useProductParams from '../hooks/useProductParams';
import useProductFunctions from '../hooks/useProductFunctions';

const ProductDetailPage = () => {
  const { product, setProduct, loading, setLoading, error, setError, reviewModalOpen, setReviewModalOpen, count, setCount, selectedImage, setSelectedImage } = useProductState();
  const { productId } = useProductParams();
  const { handleAddToCart, subtractOne } = useProductFunctions();

  useProductEffect(productId, setProduct, setLoading, setError);
            ...
            <div className="mt-8">
              <div className="grid grid-cols-2 gap-5">
                <div className='flex items-center bg-gray-100 rounded-md p-3 shadow-md'>
                  <button onClick={() => subtractOne(count, setCount)} className="px-4 h-full flex items-center justify-center text-orange-500">
                    <FaMinus />
                  </button>
                ...
                <div className=''>
                  <button
                    ...
                    onClick={() => handleAddToCart(productId, count, product)}
                  >
                    <svg
